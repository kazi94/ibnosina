	...
	function () {
		...
		$("#bout_01").click(function (e) {
			...
			$.ajax({
				...
				datatype: 'json',
				success: function (data, status) { //status = 'success'
					if (status == "success") {
						if (data != "") {
		...
		$('#regle').change(function () { //fonction pour alterner entre les deux type de règles
			if ($(this).val() == "mmte") {
				$('.mmte').toggle();
				$('.element').toggle();
			} else {
				...
				source: function (request, response) {
					$.ajax({
						...
						success: function (data, status, code) {
							response($.map(data.slice(0, 10), function (item) { // slice cut number of element to show
								return {
									label: item.cph_nom, // pour afficher dans la liste des suggestions
		...
		$('.fa-edit').click(function () {
			...
			$.ajax({ //Récupérer l'enregistrement via l'id
				...
				success: function (data, status) {
					...
					$("#inf").val(data[0].inf);
					$("#sup").val(data[0].sup);
					// $.each(data,function(i , value){ // Un ou plusieurs médicaments
