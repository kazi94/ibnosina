$(
	function () {
		...
		$('#save_01').click(function () {
			...
			var analyse_auto;
			if ($("#analyse_auto").is(':checked')) analyse_auto = 'on';
			else analyse_auto = 'off';
			// var matin_midi = $("#matin_midi").val();
			// var midi_soir = $("#midi_soir").val();
			// var soir_avant_coucher = $("#soir_avant_coucher").val();
			var settings = new Object();
			settings.hopital = hopital;
			settings.service = service;
			settings.analyse_auto = analyse_auto;
			// settings.matin_midi = matin_midi;
		...
		$.getJSON("/js/json/general.json", function (obj) {
			...
			$.each(obj, function (key, value) {
				...
				if (value.grade != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(0) ").html("<b>" + value.grade + (value.grade != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			...
			$.each(obj, function (key, value) {
				...
				if (value.specialite != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(1) ").html("<b>" + value.specialite + (value.specialite != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			});
			var i = -1;
			$.each(obj, function (key, value) {
				if (value.bilan == "") {
				...
				if (value.bilan != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(2) ").html("<b>" + value.bilan + (value.bilan != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			...
			$.each(obj, function (key, value) {
				...
				if (value.unite != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(3) ").html("<b>" + value.unite + (value.unite != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			});
			var i = -1;
			$.each(obj, function (key, value) {
				if (value.type_effet == "") {
				...
				if (value.type_effet != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(4) ").html("<b>" + value.type_effet + (value.type_effet != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			});
			$.each(obj, function (key, value) {
				if (value.type_rdv == "") {
					key++;
				...
				if (value.type_rdv != "")
					$("#settings_list>tbody>tr:eq(" + (++i) + ")> td:eq(5) ").html("<b>" + value.type_rdv + (value.type_rdv != "" ? "<i class='fa fa-times-circle' style='color:red;cursor : pointer;'></i>" : "</b>"));
			});
		...
		$('#save_02').click(function () {
			if ($('tbody>tr').length > 0)
				$('#settings_list>tbody>tr').each(function (index) {
