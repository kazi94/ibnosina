  ...
  if ($("#modal_modifier #nom").val() == "") {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (patient.lastPoids != undefined)
          $("#poids").val(patient.lastPoids.poids);
        ...
        $("#situation_familliale")
          .val(patient.situation_familliale)
          .is(":selected");
        ...
        if (
          patient.travaille == "Retraité" ||
          patient.travaille == "Universitaire"
        ) {
          $("#travaille")
            .val(patient.travaille)
            .is(":selected");
...
function setMapping(mapArray, patient) {
  return $.map(mapArray, function (obj) {
    ...
    for (let index = 0; index < patient.length; index++) {
      const pathologie_p = patient[index];
