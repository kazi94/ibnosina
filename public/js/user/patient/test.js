...
$(function(){
	...
	$(".up_patient").click(function(){
        ...
        $.ajax({
            ...
            success : (data) => {
                ...
                pathologies = setMapping(pathologies , patient.pathologies);
                allergies   = setMapping(allergies   , patient.allergies);          
                
                $('.pathologies').select2({data : pathologies});
                $('.allergies').select2({data : allergies});
                ...
                $("#situation_familliale").val(patient.situation_familliale).is(':selected'); if (patient.situation_familliale == "Marié(e)") {$("#nbre").val(patient.nbre_enfants) ;$("#nbre_enfants").show(); }           
                if (patient.travaille == "Retraité" || patient.travaille == "Universitaire")
                    {
                        $("#travaille").val(patient.travaille).is(':selected');
                        $("#travaille1").val(patient.travaille);
                    }else { $("#travaille1").val(patient.travaille);  $("#autre").show(); }
                if (patient.tabagiste  == "on") { $("#tabac").prop('checked' , true);  $("#tabac1").val(patient.tabagiste_depuis);   $(".tabac").show();  }             
                if (patient.alcoolique == "on") { $("#alcool").prop('checked' , true); $("#alcool1").val(patient.alcoolique_depuis); $(".alcool").show(); }             
                if (patient.drogue     == "on") { $("#drogue").prop('checked' , true); $("#drogue1").val(patient.drogue_depuis);     $("#type_dr").val(patient.details); $(".drogue").show(); } 
                ...
                $("#famille_antecedants").val(patient.famille_antecedants); 
                // Hospitalisation du patient
                // $("#date_admission").val(patient.hospi.date_admission); 
                // $("#date_sortie").val(patient.hospi.date_sortie);   
                // $("#chambre").val(patient.hospi.chambre);   
                // $("#lit").val(patient.hospi.lit);   
                // $("#num_biais").val(patient.hospi.num_biais);   
                // console.log(patient.hospi.service);                         
            },
    ...
    function setMapping(mapArray , patient) {
        return $.map(mapArray, function (obj) {
            ...
            $.each(patient, function(index, val) {
                //if id match , set selected 'true' 
                obj.selected = val.id == obj.id ? true : '';
            });
            return obj; 
        }); 
    }
    ...
    $("form").on('change','#date_naissance',function(){
		...
		function myFunction() {
          ...
          function myFunctions() {
            ...
            var table3 = document.getElementById("labelforma");
			// If the checkbox is checked, display the output text
			if (checkBox.checked == true){
              text.style.display = "block";
          ...
          function myFunctions2() {
			// Get the checkbox
			var checkBox = document.getElementById("scales2");
			// Get the output text
			var text = document.getElementById("labelfor2");
            var table3 = document.getElementById("labelforma2");
			// If the checkbox is checked, display the output text
			if (checkBox.checked == true){
              text.style.display = "block";
              table3.style.display="none";
          ...
          function myFunctions3() {
			// Get the checkbox
			var checkBox = document.getElementById("scales3");
			// Get the output text
			var text = document.getElementById("labelfor3");
            var table3 = document.getElementById("labelforma3");
			// If the checkbox is checked, display the output text
			if (checkBox.checked == true){
              text.style.display = "block";
              table3.style.display="none";
        ...
        $(document).ready(function() {
			...
			$.ajax({
				...
				success : function (data) {
                    ...
                    jsonData.push({id:JSONObject[0][i].id,name:JSONObject[0][i].element})
                    }
        ...
        $(document).ready(function() {
			...
			$.ajax({
				...
				success : function (data) {
                    var JSONObject = JSON.parse(JSON.stringify(data));
                    for (var i =0; i < JSONObject[0].length; i++) {
                    jsonData.push({id:JSONObject[0][i].id,name:JSONObject[0][i].nom})
                    }
