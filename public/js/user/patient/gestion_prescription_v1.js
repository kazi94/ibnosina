	...
	function()
	{
			...
			$.get({
				...
				success : function(data){
					...
					for (var i = 0; i < lignes.length; i++) 
					{
					 	...
					 	$ipMsg = ips[i].ip;
						$("#tablePrescription>tbody").prepend(
							"<tr data-toggle=\'tooltip\' data-placement='top'  title='"+$ipMsg+"' >"+ 
							"<td colspan='2'><input type='hidden' name='last_presc_id' value="+$prescription_id+" />"+ 
								"<input type='hidden' name='med_sp_id[]' value="+lignes[i][0].med_sp_id+" />"+lignes[i][1]+
							"</td>"+
							"<td width='80px' >"+
								"<input type='hidden' name='voie[]' value="+lignes[i][0].voie+" />"+lignes[i][0].voie+"</td>"+
							"<td width='80px'> <input type='text' class='form form-control' name='dose[]' value='"+$dose +"'></td>"+ 
							"<td><input type='checkbox' class='form-control flat-green' name='dose_matin[]'  "+$matin    +" /></td>"+ 
							"<td><input type='checkbox' class='form-control flat-green' name='dose_midi[]'   "+$midi     +" /></td>"+ 
							"<td><input type='checkbox' class='form-control flat-green' name='dose_soir[]'   "+$soir     +" /></td>"+ 
							"<td><input type='checkbox' class='form-control flat-green' name='dose_av[]'     "+$av       +" /></td>"+ 
							"<td width='97px'>"+ 
								"<select class='form form-control' name='unites[]' >"+
								    "<option selected>"+lignes[i][0].unite+"</option>"+ 
								"</select>"+ 
							"</td>"+ 
							"<td> <input type='text' class='form-control' name='nbr_jours[]' value="+lignes[i][0].nbr_jours+"> </td>"+ 
							"<td> <select name='type_j[]' class='form-control'> <option selected>jours</option> <option>semaines</option> <option>mois</option> </select> </td>"+ 
							"<td style='text-align:center;'><i class='fa fa-times-circle' style='color:red;cursor : pointer;' onclick='$(this).closest(\"tr\").remove();'></i></td>"+
							"</tr>");
						$('input[type="checkbox"].flat-green').iCheck({checkboxClass: 'icheckbox_flat-green'});
						// $('[data-toggle="tooltip"]').tooltip()
					}						
					$("#modal_prescription").modal({ show: true });
				}
			});
		...
		$("a.deleteRowPrescription").click(function(event) {
				if (confirm('voulez vous supprimer cette ligne ?')) {
					...
					$.ajax({url: url, type: 'POST', method : 'DELETE', headers : {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					}).done((response) => {
