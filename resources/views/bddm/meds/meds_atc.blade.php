...
@section('title') Classes ATC | HIC MEDIC @endsection
@section('description') la liste des classes ATC. @endsection
@section('og_title') Classes ATC | HIC MEDIC @endsection
@section('og_description') la liste des classes ATC. @endsection
            ...
            <div id="accordion" style="color: red">
              @foreach ($r as $classe)
                  <div class="card" style="color: red">
                    <div class="card-header" style=" padding:0; "id="heading-{{ $classe->CATC_CODE_PK}}">
                      <span class="mb-0">
                        <button class="btn btn-link collapsed" style="color: black" data-id="{{ $classe->CATC_CODE_PK}}" data-toggle="collapse" href="#collapse-{{ $classe->CATC_CODE_PK}}" aria-expanded="false" aria-controls="collapse-{{ $classe->CATC_CODE_PK}}">
                          <i class="fa fa-plus-circle"></i> {{ $classe->CATC_NOMF}}
                        </button>
                      </span>
                    </div>
