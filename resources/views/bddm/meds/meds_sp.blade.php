...
@section('title') Médicaments | HIC MEDIC @endsection
@section('meta_robot') index,follow @endsection
@section('description')
la liste des médicaments. Environ 21 000 spécialités diponnibles.@endsection
@section('og_title') Médicaments | HIC MEDIC @endsection
	...
	<div class="row d-flex justify-content-center">
        ...
        <button   title="Signaler un problème" type="button" class="btn btn-link btn-report" data-toggle="modal" data-target="#bug_report" ><i class="fa fa-bug"></i> Signaler !</button>
			<div class="abecedaire">
                {{-- <h1>Recherche d'un médicament  commençant par A</h1> --}}
                {{-- <div class="update_date" itemscope="itemscope" itemtype="http://schema.org/MedicalWebPage">Mis à jour : <span itemprop="lastReviewed">Mardi 17 Septembre 2019</span></div> --}}
...
{{--                     @foreach ($r as $el)
                                    <tr>
                                        <td> <a href="/medicaments/{{ $el->SP_CODE_SQ_PK }}">{{ $el->SP_NOM }}</a> </td>
                                    </tr>
                                @endforeach        --}}     
                </tbody>			
			</table>					
