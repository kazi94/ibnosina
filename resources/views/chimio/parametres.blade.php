 ...
 <div class="row">
  ...
  <div class="tab-control-wrap">
    <ul class="tab-control">
      ...
      <li><a href="{{(route('listMaladie.index'))}}">lister les maladies</a></li>
    </ul>
  </div>
  <div class="tabs">
    <div class="tab" id="tab-1" style="">
      <div class="col-sm-12">
       ...
       <form action="{{ route('activerFormule') }}" method="post" id="lk">
                      {{ csrf_field() }}
       ...
       <div class="form-group" style=" width: 40%;position: relative; left: 30%;">
              ...
              <select class="form form-control" name="formule" id="formule" >  
                @foreach($formules as $f)
                  @if($formule_utilise != null && $formule_utilise->id == $f->id)
                    <option selected style="background-color: #c2f8ac" value="{{$f->id}}">{{$f->formule}}</option>
                  @else
                    <option value="{{$f->id}}">{{$f->formule}}</option>
                  @endif
                @endforeach
              </select><br>
...
<form action="{{ route('addFormule') }}" method="post" id="kk">
                      {{ csrf_field() }}
    ...
    <div class="tab" id="tab-2">
     ...
     <div class="col-sm-3"></div>
        <div class="col-sm-6">
                    <div class="card" >
                        <div class="header">
                             <form action="{{ route('addunite') }}" method="post" id="unite">
                                        {{ csrf_field() }}
                                ...
                                <tbody>
                                  @foreach($unites as $u)
                                    <tr>
                                        ...
                                        <td>{{$u->intitule}}</td>
                                        <td>{{$u->unite}}</td>
                                        <td><span class="glyphicon glyphicon-trash" style="color: red; cursor: pointer" onclick="deleteLigne({{$u->id}})"></span></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <table id="t_protocole" class="table table-responsive text-center dataTable table table-hover" aria-describedby="t_protocole">
                                ...
                                <tbody>
                                  @foreach($para as $p)
                                     <tr>
                                        <th>{{$p->id}}</th>
                                        <td>{{DB::table('sp_specialite')->where('SP_CODE_SQ_PK',$p->medicament_id)->pluck('SP_NOM')->first()}}</td> 
                                        <td>{{$p->volume_medi}} ML</td>
                                        <td>{{$p->solvant_recon}}</td>
                                        <td>{{$p->solvant_dilu}}
          ...
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <div class="modal-body">   
                <form action="{{ route('addpara') }}" method="post">
                            {{ csrf_field() }}
                <div class="box-body">
